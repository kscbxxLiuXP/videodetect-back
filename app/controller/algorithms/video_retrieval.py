...
from app.controller.algorithms.ImageNNExtract import *
from app.controller.algorithms.retrieval_util import *
...
def retrieval(query_numpy_path, candidate_refer_numpy_path, query_frame_2_time=None, refer_frame_2_time=None,
    ...
    for k, (_, r_vid) in enumerate(r_score):
        ...
        if len(path_q) > 0:
            # r_vid:D:/大创/项目代码/website/data/VideoDB/feature/1260706600.npy
            time_q = [query_frame_2_time[qf_id + 1] for qf_id in path_q]
            time_r = [refer_frame_2_time[r_vid.split('.')[0].split('/')[-1]][str(rf_id + 1)] for rf_id in path_r]
            q_ans.append((score, r_vid, time_q[0], time_q[-1], time_r[0], time_r[-1]))
    q_ans.sort(key=lambda x: x[0], reverse=True)
    return q_ans[0]


def video_retrieval(input_video_npy, feature_db, query_frame_2_time, refer_frame_2_time):
    t = retrieval(input_video_npy, feature_db, query_frame_2_time, refer_frame_2_time)
    return t
